...
export default function CreateJobForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success && res.job) {
      router.push(`/company/${companySlug}/jobs/${res.job.id}`);
      router.refresh();
    } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-white">Create New Job Posting</h1>
            <p className="text-xs text-slate-400">Post a new position for your company careers portal</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div className="sm:col-span-2 space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 font-semibold"
            />
          </div>
          ...
          <div className="space-y-1.5 flex items-center pt-5">
            <label className="flex items-center gap-2 cursor-pointer text-xs font-bold text-slate-200">
              ...
              />
              <span>Publish position immediately</span>
            </label>
          </div>
          ...
          <div className="sm:col-span-2 space-y-1.5">
            ...
            <textarea
              rows={8}
              placeholder="Describe the role, key responsibilities, qualifications, and benefits..."
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              className="w-full bg-slate-900 border border-slate-700 rounded-lg p-4 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
            />
          </div>
        </div>
